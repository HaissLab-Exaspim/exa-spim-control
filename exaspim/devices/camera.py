...
class Camera:
    ...
    def configure(self):
        ...
        self.grabber.realloc_buffers(
            self.cfg.egrabber_frame_buffer
        ...
        self.grabber.remote.set(
            "Width", int(self.cfg.sensor_column_count)
        )  # set roi width
        ...
        self.grabber.remote.set(
            "OffsetX",
            round((width_x / 2 - self.cfg.sensor_column_count / 2.0) / 16) * 16,
        )  # center roi on sensor, multiple of 16
        ...
        self.grabber.remote.set(
            "ExposureTime", round(self.cfg.camera_dwell_time * 1.0e6, 1)
        )
    ...
    def grab_frame(self):
        ...
        with Buffer(self.grabber, timeout=timeout_ms) as buffer:
            ...
            image = numpy.frombuffer(
                data,
                count=int(self.cfg.sensor_column_count * self.cfg.sensor_row_count),
                dtype=numpy.uint16,
            ).reshape((self.cfg.sensor_row_count, self.cfg.sensor_column_count))
    ...
    def collect_background(self, frame_average=1):
        ...
        if (
            self.grabber.remote.get("TriggerMode") != "Off"
        ):  # set camera to internal trigger mode
            self.grabber.remote.set("TriggerMode", "Off")
        # Initialize background image array
        bkg_image = numpy.zeros(
            (frame_average, self.cfg.sensor_row_count, self.cfg.sensor_column_count),
            dtype="uint16",
        )
    ...
    def get_camera_acquisition_state(self):
        ...
        state = {}
        state["frame_index"] = self.grabber.stream.get_info(
            STREAM_INFO_NUM_DELIVERED, INFO_DATATYPE_SIZET
        )
        state["in_buffer_size"] = self.grabber.stream.get_info(
            STREAM_INFO_NUM_QUEUED, INFO_DATATYPE_SIZET
        )
        state["out_buffer_size"] = self.grabber.stream.get_info(
            STREAM_INFO_NUM_AWAIT_DELIVERY, INFO_DATATYPE_SIZET
        )
        state["dropped_frames"] = self.grabber.stream.get_info(
            STREAM_INFO_NUM_UNDERRUN, INFO_DATATYPE_SIZET
        )  # number of underrun, i.e. dropped frames
        state["data_rate"] = self.grabber.stream.get(
            "StatisticsDataRate"
        )  # stream data rate
        state["frame_rate"] = self.grabber.stream.get(
            "StatisticsFrameRate"
        )  # stream frame rate
        self.log.debug(
